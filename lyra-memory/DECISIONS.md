...
## Step 6 — "correlates with outcome valence": magnitude, not sign

**Ambiguous:** the verification target is "salience correlates with outcome
valence — direction, not magnitude". Which direction is left open, and the two
readings give opposite systems.

**Chosen:** salience tracks |valence|. A strongly negative outcome is exactly
as salient as a strongly positive one.

**Why:** the spec says memory keeps "discontinuities only: first success, the
failure that changed approach". Both are extremes; only one is positive. If
salience tracked signed valence she would preferentially forget what went
wrong, which is the opposite of what a track record is for, and it would hand
the forgetting pass a bias nobody chose.

**Alternative:** signed valence, so successes outrank failures.

**Made explicit rather than implicit:** a test asserts the signed covariance is
~0 and that salience is monotonic in |valence|, so the property is defended
rather than left to be rediscovered later as a bug and "fixed".

**Reversal:** drop the `ABS()` in the pass. Read the test that then fails
before deciding that is what you want.

## Step 6 — session spillover, and a nonzero baseline

**Ambiguous:** the sheet says the pass reads "atoms, outcomes" without saying
how far an outcome's weight reaches.

**Chosen:** three layers. Every atom starts at a nonzero baseline; atoms in a
session containing an outcome get a fraction of that outcome's magnitude; the
attempt's own atom gets the full value.

**Why a nonzero baseline:** at zero, every atom with no measured outcome is
equally forgettable, and the forgetting pass has nothing to rank them by.

**Why spillover at all:** what made an outcome make sense is usually the
exchange around it, not the single line that triggered it.

**Reversal:** set `SALIENCE_SESSION_SPILLOVER` to 0 for attempt-only scoring.

## Step 7 — two extractors, because they fail in opposite directions

**Ambiguous:** the sheet says entity extraction reads `atoms.text` and does not
say how.

**Chosen:** a model for people, projects and topics; a narrow regex for file
paths and repo names. Both write into the same tables.

**Why both:** exact strings are what an index most needs verbatim and what a
model is least reliable at reproducing — it re-cases them, paraphrases them,
and invents slashes. Meanwhile no regex finds a project or a person. A test
asserts a file path is still extracted when the model returns nothing at all.

**Reversal:** either half can be removed independently.

## Step 7 — entity extraction *does* read `sandbox_read`

**Ambiguous:** dream excludes document-sourced atoms; nothing says whether
entity extraction should too.

**Chosen:** it reads them. Documents produce facts, facts are keyed on
entities, so excluding documents here would quietly break fact retrieval for
anything she learned by reading.

**Why this is not a hole in the boundary:** the rule is dispositional, not
subject-based. Nothing in this pass writes to dispositional state, and
`atom_entities` keeps provenance back to the atom the name came from.

**Reversal:** add the same `NOT IN (excluded_sources)` clause the dream pass
uses. Note what breaks: facts about anything she has only read about.

## Step 8 — `source_kind` is decided by the channel, never by the model

**Ambiguous:** `source_kind` is a column with a vocabulary; the sheet does not
say who assigns it.

**Chosen:** derived from the atom — its `source` first, then its `speaker`.
The extracting model's opinion is discarded even when it offers one.

**Why the document check comes first:** it does not matter who was speaking
when a file was read. A model that could nominate `source_kind="stated"` for a
document would launder a file's authority into something she treats as
first-hand, which is exactly the injection surface the `source_kind` column
exists to keep visible.

**Reversal:** none wanted.

## Step 9 — a vague deadline does not become a `due_ts`

**Ambiguous:** "due_ts REAL NULL — only if explicit". "by friday" is explicit
as a phrase but not as a timestamp.

**Chosen:** `due_ts` is set only when a real timestamp is available. "by
friday" stays in the commitment text.

**Why:** overdue is a state this store reports on, so an invented deadline
produces a fabricated overdue commitment — worse than no deadline at all,
because it looks like something she was told.

**Reversal:** resolve relative dates upstream, where the conversation's actual
date is known, and pass a timestamp in.

## Step 9 — closure runs before extraction within a batch

**Ambiguous:** unspecified, and it matters when a commitment is made and
completed inside the same batch of atoms.

**Chosen:** closure first, against commitments that were already open. A
commitment made and finished in one batch is therefore recorded as made, and
closed on the next pass — rather than being closed before it exists, or
skipped.

**Reversal:** swap the two calls. The failure mode to watch for is a
commitment that never appears at all.

## Step 10 — connected components, not community detection

**Ambiguous:** "clustering" names no algorithm.

**Chosen:** connected components over entity co-occurrence edges above a
weight threshold.

**Why:** community detection needs a resolution parameter, which is a tuning
constant with no data yet to fit it to. An over-merged cluster is visible and
fixable; a grouping nobody can explain is neither. Co-occurrence edges are
countable, so every project traces back to the atoms that built it.

**Why co-occurrence rather than embedding similarity:** similarity would group
`lyra-voice` with `lyra-listen` because their names are alike, which is a fact
about spelling. Co-occurrence groups them because they keep appearing
together, which is a fact about her history.

**Reversal:** swap `_components()` for a community algorithm once there is
enough data to choose a resolution against.

## Step 10 — projects are upserted, never deleted

**Chosen:** project entities are upserted by name and only their own atom links
are rebuilt. A cluster that stops appearing leaves its project entity behind.

**Why:** `facts.subject` may already key on that name, and this store has no
deletion path. Projects are also excluded from their own input, so clusters
cannot merge with themselves on each run.
